...
import { interceptorsForResponses, responsesOptionalInterceptors } from './index.ts';
...
test('interceptorsForResponses on provider with Copilot interceptors and retry-cyber-policy', () => {
  ...
  assertEquals(assembled, [...responsesCopilotInterceptors, withCyberPolicyRetried]);
});
...
test('interceptorsForResponses ignores unknown enabledFixes silently at the assembler layer', () => {
  ...
  const ids: readonly string[] = responsesOptionalInterceptors.map(d => d.fixId);
