...
test('messagesResultToEvents preserves protocol citation url fields', () => {
  ...
  const citationFrame = frames.find(frame => frame.event.type === 'content_block_delta' && frame.event.delta.type === 'citations_delta');

  assertEquals(citationFrame?.event.type === 'content_block_delta' && citationFrame.event.delta.type === 'citations_delta' ? citationFrame.event.delta.citation : undefined, {
    type: 'search_result_location',
    url: 'https://example.com/protocol',
    title: 'Protocol Citation',
    search_result_index: 0,
...
test('messagesResultToEvents omits signature deltas for text-only thinking blocks', () => {
  ...
  assertEquals(
    frames.filter(frame => frame.event.type === 'content_block_delta').map(frame => frame.event),
    [
      {
