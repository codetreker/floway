...
const claudeVariant = (
  ...
  ...(overrides.multiplier !== undefined || overrides.restrictedTo !== undefined
    ? {
        billing: {
          is_premium: true,
          ...(overrides.multiplier !== undefined ? { multiplier: overrides.multiplier } : {}),
          ...(overrides.restrictedTo !== undefined ? { restricted_to: overrides.restrictedTo } : {}),
        },
      }
    : {}),
});
...
test('mergeClaudeVariants merges 4.7 base + high + xhigh + 1m-internal', () => {
  ...
  assertSameSet(m.billing?.restricted_to, ['enterprise', 'pro_plus', 'business', 'max']);
});
...
test('mergeClaudeVariants merges 4.6 base + 1m', () => {
  const input: CopilotModelsResponse = {
    ...
    data: [
      ...
      claudeVariant('claude-opus-4.6', {
        ...
        maxOutputTokens: 32_000,
        multiplier: 3,
        restrictedTo: ['pro', 'pro_plus', 'individual_trial', 'business', 'enterprise', 'max'],
      }),
    ],
  };
  ...
  assertSameSet(m.billing?.restricted_to, ['pro', 'pro_plus', 'business', 'enterprise', 'max', 'individual_trial']);
});
